...
void getpassword(struct Account* login) {
	...
	do {
		...
		for (i = 0; i < 8; i++) {
			...
			if (login->loggin.password[i]=='!'|| login->loggin.password[i] == '@' || login->loggin.password[i] == '#' || login->loggin.password[i] == '$' || login->loggin.password[i] == '%' || login->loggin.password[i] == '^' || login->loggin.password[i] == '&' || login->loggin.password[i] == '*' ) {// check if the symbol is !@#$%^&*
				flagpunc++;
			}
		}
